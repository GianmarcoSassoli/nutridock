...
| Backend API | **Fastify** + Zod | Leggero, veloce, validazione schema-first; NestJS sarebbe overkill per un progetto personale |
| ORM / DB | **Drizzle ORM** + **PostgreSQL 16** con estensione **TimescaleDB** | Relazionale per dispensa/ricette/obiettivi; hypertable Timescale per le serie temporali (pesate, calorie) |
| Job scheduler | **BullMQ** (Redis) o semplice `node-cron` | Sync periodici Yazio, generazione menù notturna. Per iniziare basta node-cron; BullMQ se servono retry robusti |
| Frontend | **React + Vite + Tailwind + Recharts**, PWA | SPA servita dallo stesso container o da nginx; PWA per averla sul telefono |
| Messaging | **Mosquitto (MQTT)** | Bus già naturale nell'ecosistema HA/ESPHome; ble-scale-sync pubblica direttamente su MQTT |
| Bilancia | **ble-scale-sync** (container o add-on HA) | Progetto Node.js GPL-3.0 con 26 adapter di protocollo, incluso ICOMON (chipset delle bilance Fitdays/Healthkeep) e calcolo body composition derivato da openScale |
...
```
body_metrics(ts, weight, impedance, fat_pct, muscle_kg, water_pct, bone_kg,
             visceral_fat, bmr, protein_pct, metabolic_age, source)   -- hypertable
food_log(ts, meal, product_id, name, qty_g, kcal, carbs, protein, fat,
         source[yazio|voice|manual], yazio_entry_id)                  -- hypertable
activity(date, active_kcal, resting_kcal, steps, exercise_min, source) 
goals(user_id, target_kcal_deficit, protein_g, carbs_g, fat_g, target_weight, valid_from)
...
1. Deploy come container Docker sul NAS (o add-on se HA è HAOS).
...
Metodologia [openScale wiki](https://github.com/oliexdev/openScale/wiki/How-to-reverse-engineer-a-Bluetooth-4.x-scale):

1. Su Android: abilita *Bluetooth HCI snoop log*, esegui una pesata con Fitdays, estrai il log e analizzalo in Wireshark. In alternativa nRF Connect per esplorare servizi/characteristic GATT.
2. Identifica la characteristic che notifica peso/impedenza (tipicamente vendor UUID `0xFFFx` o standard `0x2A9C` Body Composition Measurement) e decodifica il payload (endianness, scala /100, flag di misura stabilizzata).
3. Implementa un **adapter custom per ble-scale-sync** (architettura a plugin, [guida contributing](https://github.com/KristianP26/ble-scale-sync/blob/main/CONTRIBUTING.md)) — così resti dentro l'infrastruttura del Livello 1 e lo contribuisci upstream.
...
|---|---|---|---|
| **0 — Fondamenta** | Monorepo pnpm, `docker-compose` sul NAS (Postgres+Timescale, Redis, Mosquitto), schema Drizzle, CI locale | 1 weekend | `docker compose up` funziona, migrazioni ok |
...
| **2 — Bilancia** (in parallelo a 1) | ESP32 flashato come ESPHome BT Proxy, ble-scale-sync container, scan+config MAC, exporter MQTT+webhook → `body_metrics` | 1–2 sere (L1); +1 weekend se serve L2 | Una pesata appare nel DB con tutte le metriche |
